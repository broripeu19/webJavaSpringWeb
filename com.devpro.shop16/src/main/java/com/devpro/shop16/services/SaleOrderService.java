...
public class SaleOrderService extends BaseService<SaleOrder> {
	...
	@Transactional
	public SaleOrder confirmSaleOrder(int saleOrderId) {
		String sql = "update SaleOrder s set s.status = 0 where s.id = :id";
		entityManager.createQuery(sql)
				.setParameter("id", saleOrderId)
				.executeUpdate();
		return entityManager.find(SaleOrder.class, saleOrderId);
	}
}
