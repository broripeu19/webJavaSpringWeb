...
// Cập nhật số lượng sản phẩm vào trong giỏ hàng
function UpdateQualityCompare(baseUrl, productId) {
	// javascript object.  data la du lieu ma day len action cua controller
	let data = {
		productId: productId, // lay theo id
	...
	// json == javascript object
	jQuery.ajax({
		url: baseUrl + "/ajax/updateQualityCompare", //->action
		type: "post",
		contentType: "application/json",
		data: JSON.stringify(data),
...
// Cập nhật số lượng sản phẩm vào trong giỏ hàng
function RemoveQualityCompare(baseUrl, productId) {
	// javascript object.  data la du lieu ma day len action cua controller
	let data = {
		productId: productId, // lay theo id
	...
	// json == javascript object
	jQuery.ajax({
		url: baseUrl + "/ajax/RemoveQualityCompare", //->action
		type: "post",
		contentType: "application/json",
		data: JSON.stringify(data),
...
// Thêm sản phẩm vào trong giỏ hàng
function AddToCompare(baseUrl, productId, quanlity) {
	// javascript object.  data la du lieu ma day len action cua controller
	let data = {
		productId: productId, // lay theo id
		quanlity: quanlity, // lay theo id
	...
	// json == javascript object
	jQuery.ajax({
		url: baseUrl + "/ajax/addToCompare", //->action
		type: "post",
		contentType: "application/json",
		data: JSON.stringify(data),
		...
		success: function(jsonResult) {
			// tăng số lượng sản phẩm trong giỏ hàng trong icon
			$( "#iconShowTotalItemsInCompare" ).html(jsonResult.totalItems);
			$([document.documentElement, document.body]).animate({
				scrollTop: $("#iconShowTotalItemsInCompare").offset().top
			}, 2000);
		},
		error: function(jqXhr, textStatus, errorMessage) {
